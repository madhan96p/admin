document.addEventListener("DOMContentLoaded", () => {
  ...
  const handleSearch = () => {
    ...
    const results = allSlips.filter((slip) => {
      const dsNoMatch = dsNoQuery
        ? (slip["DS_No"] || "").toLowerCase().includes(dsNoQuery)
        : true;
      const guestMatch = guestQuery
        ? (slip["Guest_Name"] || "").toLowerCase().includes(guestQuery)
        : true;
      const slipDateFormatted = slip["Date"]
        ? new Date(slip["Date"]).toISOString().split("T")[0]
        : "";
  ...
  const displayResults = (slips) => {
    ...
    setTimeout(() => {
      ...
      slips.forEach((slip, index) => {
        const statusClass = (slip.Status || "New")
          .toLowerCase()
          .replace(/ /g, "-");
        ...
        card.innerHTML = `
                    ...
                    <div class="slip-card-body">
                        ...
                        <div class="info-item"><i class="fas fa-calendar-alt"></i><div class="info-item-content"><p><strong>${formatDate(slip.Date)}</strong>Date</p></div></div>
  ...
  // --- 6. EVENT LISTENERS ---
  document.getElementById("find-slip-btn").addEventListener("click", (e) => {
    e.preventDefault();
    handleSearch();
  });
  ...
  resultsContainer.addEventListener("click", (e) => {
    ...
    if (target.classList.contains("copy-btn")) {
      navigator.clipboard
        .writeText(window.location.origin + target.dataset.link)
      ...
      switch (shareType) {
        case "driver":
          ...
          break;
        case "guest-info":
          shareInfoWithGuest(slipData);
          break;
        case "guest-close":
          askGuestToClose(slipData);
          break;
        case "thank-you":
          sendThankYouToGuest(slipData);
          break;
      }
    }
  });
