document.addEventListener('DOMContentLoaded', () => {
        ...
        slips.forEach(slip => {
            ...
            slipCard.innerHTML = `
                <div class="slip-summary">
                    <h3>DS #${slip['DS_No']}</h3>
                    <p><strong>Guest:</strong> ${slip['Guest Name']} | <strong>Driver:</strong> ${slip['Driver Name']}</p>
                </div>
    ...
    resultsContainer.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('copy-btn')) {
            ...
            const slipData = allSlips.find(s => s['DS_No'] === target.dataset.id);
            handleQuickShare(target.dataset.type, slipData);
        }
    });
