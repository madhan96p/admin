document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 5. CORE HANDLERS ---
    // In create-slip.js, replace the entire handleSave function
    // In close-slip.js, replace the entire updateDutySlip function
...
async function updateDutySlip(event) {
    ...
    // This is the fix: We only send the fields a driver is allowed to change.
    const dataToUpdate = {
        DS_No: slipId,
    ...
    try {
        const response = await fetch('/api?action=updateDutySlip', {
            ...
            body: JSON.stringify(dataToUpdate) // Send the smaller, safer object
        });
    ...
    function handleWhatsAppShare() {
        ...
        if (shareWith && shareWith.toLowerCase() === 'driver') {
            ...
            // In handleWhatsAppShare, change the generateLink function
            const dsNo = document.getElementById('ds-no').value;
            const link = `${window.location.origin}/close-slip.html?id=${dsNo}`;
            const message = `
*Duty Details*
