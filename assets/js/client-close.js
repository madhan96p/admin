document.addEventListener("DOMContentLoaded", () => {
    ...
    if (el) {
      if (el.tagName === "IMG") {
        ...
        if (value && value.length > 100) {
          // Simple check for non-empty base64
          el.src = value;
  ...
  async function loadInitialData() {
    if (!slipId) {
      loader.innerHTML =
        '<p style="color: var(--danger-color);">Error: No Duty Slip ID provided.</p>';
      return;
    }
    ...
    try {
      ...
      populateField(
        "ds-date",
        `Date: ${slip.Date || new Date().toLocaleDateString("en-GB")}`,
      );
  ...
  async function handleClientSave(event) {
    ...
    try {
      ...
      if (result.success) {
        ...
        document.getElementById("thank-you-greeting").textContent =
          `Thank You, ${guestName}!`;
