...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function render() {
    ...
    emptyState.style.display =
      !isLoading && !error && allReviews.length === 0 ? "block" : "none";
    grid.style.display =
      !isLoading && !error && allReviews.length > 0 ? "grid" : "none";
  ...
  async function openViewModal(reviewId) {
    modalBody.innerHTML =
      '<div class="skeleton-loader"><div class="skeleton-card" style="height: 200px;"></div></div>';
    viewModal.classList.add("visible");

    try {
      const response = await fetch(
        `/api?action=getFeedbackDetails&id=${reviewId}`,
      );
      const data = await response.json();
