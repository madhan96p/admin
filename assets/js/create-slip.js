...
function initializeCreateSlipPage() {
  ...
  function setupEventListeners() {
    ...
    document
      ...
      .forEach((btn) => btn.addEventListener("click", handleSave));

    document
      .querySelectorAll("#download-pdf-button, #mobile-download-pdf-button")
      .forEach((btn) => {
    ...
    document
      ...
      .forEach((el) => el.addEventListener("input", validateAllInputs));
    ...
    document
      .getElementById("reporting-time")
      ?.addEventListener("change", (e) => {
        if (e.target.value) {
          const [h, m] = e.target.value.split(":");
          const date = new Date();
          date.setHours(parseInt(h), parseInt(m) + 50);
          document.getElementById("driver-time-out").value =
  ...
  async function handleSave(event) {
    ...
    if (!validateAllInputs()) {
      return alert(
        "Please fix the validation errors (marked in red) before saving.",
      );
    }
