document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadAndRenderInvoice() {
    try {
      const response = await fetch(
        `/.netlify/functions/api?action=getInvoiceByPublicId&pid=${publicId}`,
      );
  ...
  function populateMeta(invoice) {
    ...
    const formattedTimestamp = (invoice.Last_Updated || "N/A").replace(
      /:\d\d$/,
      "",
    );
  ...
  function populateSummary(invoice) {
    ...
    setText(
      "trip-total-duration",
      `${invoice.Total_Hours} Hrs (${invoice.Billing_Slabs} Slab/s)`,
    );
  }
