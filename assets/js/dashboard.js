...
async function fetchSalarySlipsData() {
  try {
    const response = await fetch(
      "/.netlify/functions/api?action=getAllSalarySlips",
    );
...
async function fetchDutySlipsData() {
  try {
    const response = await fetch(
      "/.netlify/functions/api?action=getAllDutySlips",
    );
    if (!response.ok) throw new Error("Network response was not ok.");
    const data = await response.json();
    return data.slips || [];
...
function filterAndDisplayUpcomingTrips(slips) {
  ...
  if (upcomingSlips.length === 0) {
    listContainer.innerHTML =
      '<p class="text-muted" style="text-align:center;"><i class="fas fa-calendar-check"></i> No scheduled trips in the next 7 days.</p>';
    return;
  }

  const tripListHTML = upcomingSlips
    .map((slip) => {
      const slipDate = parseGSheetDate(slip.Date);
      const dateString = slipDate.toLocaleDateString("en-IN", {
        month: "short",
        day: "numeric",
      });
...
function displayCriticalStats(slips) {
  ...
  const needsActionCount = slips.filter((slip) => {
    const slipDate = parseGSheetDate(slip.Date);
    const isClosed =
      slip.Status === "Closed by Client" || slip.Status === "Closed by Driver";
  ...
  const completedCount = slips.filter((slip) => {
    ...
    return (
      slip.Status === "Closed by Client" &&
      slipDate &&
      slipDate >= thirtyDaysAgo
    );
  }).length;
