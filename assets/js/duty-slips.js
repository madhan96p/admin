document.addEventListener("DOMContentLoaded", () => {
  ...
  const reviewGuestNameDisplay = document.getElementById(
    "review-guest-name-display",
  ); // <-- ADD THIS
  ...
  async function loadDutySlips() {
    ...
    try {
      ...
      state.allSlips = data.slips
        ? data.slips.sort((a, b) => b.DS_No - a.DS_No)
        : [];
      state.error = null;
    } catch (error) {
  ...
  function getFilteredSlips() {
    ...
    return state.allSlips.filter((slip) => {
      ...
      let matchesFilter =
        ...
        (state.activeFilter === "Needs Action" &&
          ["New", "Updated by Manager"].includes(status)) ||
  ...
  function renderTable(slips) {
    ...
    slips.forEach((slip, index) => {
      ...
      row.innerHTML = `
            ...
            <td data-label="Guest & Date"><div class="cell-primary">${slip.Guest_Name || "N/A"}</div><div class="cell-secondary">${formatDate(slip.Date)}</div></td>
  ...
  function updateStatCards() {
    ...
    statNew.textContent = state.allSlips.filter(
      (s) => (s.Status || "New") === "New",
    ).length;
    statCompleted.textContent = state.allSlips.filter(
      (s) => s.Status === "Closed by Client",
    ).length;
  }
  ...
  async function fetchGuestNameForReview() {
    ...
    reviewGuestNameDisplay.innerHTML =
      '<i class="fas fa-spinner fa-spin"></i> Fetching guest name...';
